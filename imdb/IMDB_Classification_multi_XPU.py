...
def train(train_data_loader, eval_data_loader, model, optimizer, num_epoch, log_step_interval, save_step_interval, eval_step_interval, save_path, resume=""):
    ...
    if resume != "":
        ...
        checkpoint = torch.load(resume,map_location="cpu") # "cuda" "cuda:0"
        model.load_state_dict(checkpoint['model_state_dict']) # 这个不需要加一个.module
        optimizer.load_state_dict(checkpoint['optimizer_state_dict'])
        start_epoch = checkpoint['epoch']
        start_step = checkpoint['step']

    model = nn.DataParallel(model.cuda(),device_ids=[0,1]) # 使用DataParallel做一个封装 单进程 效率较低
    model.cuda() # 模型拷贝
        ...
        for batch_index, (target, token_index) in enumerate(train_data_loader):
            ...
            if step % save_step_interval == 0:
                ...
                torch.save({
                    'epoch': epoch_index,
                    'step': step,
                    'model_state_dict': model.module.state_dict(), # 保存的时候调用model.nodule.state_dict
                    'optimizer_state_dict': optimizer.state_dict(),
                    'loss': bce_loss,
                }, save_file) # 模型保存不需要做任何修改
...
if __name__ == "__main__":
    if torch.cuda.is_available():
        ...
        if torch.cuda.device_count() > 1:
            ...
            logging.warning(f"Too few GPUs")

        os.environ["CUDA_VISIBLE_DEVICES"] = "0,1"
        # 在命令行中的方式
        # CUDA_VISIBLE_DEVICES = "0,,1" python3 IMDB_Classifition_single_XPU.py
    else:
