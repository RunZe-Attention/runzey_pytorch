...
def image2emb_convolution(image,kernel,stride):
    conv_output = F.conv2d(image, (4,4), stride=1)
...
class SwinTransformerModel(nn.Module):
    ...
    def forward(self,image):
        ...
        #patch_embbeding_naive = image2emb_convolution(image,"anyway","anyway")
        # stage1
...
def test():
    ...
    model(image)
    exportname = "swin.onnx"
    torch.onnx.export(model, image, exportname)
